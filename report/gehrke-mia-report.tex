...
One fundamental part of the aforementioned process remains open: The training of an attack model. Therefore, Shokri et al. \cite{shokri2017membership} propose a technique called "\textit{shadow training}". Multiple shadow model $f_{shadow}^{i}$ are generated with the main aim being to make their behavior as similar to the target model's behavior as possible. The training datasets $D_{train}^{shadow^{i}} are all known and may partially overlap. Training data for the shadow models may orignate from existing sources. Otherwise it has to be generated as "\synthetic" training data.
To generatet training data for the shadow models, Shokri et al. propose different methods:
